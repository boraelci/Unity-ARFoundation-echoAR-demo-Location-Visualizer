...
public class echoAR : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        try
        {
            ...
            string pin_entryID = "<PIN_ENTRY_ID>"; //<ENTRY_ID>
            LocationVisualize(url, key, pin_entryID);
    ...
    void LocationVisualize(string url, string key, string pin_entryID){
      ...
      StartCoroutine(GetGlobalMetadata(url, key, pin_entryID, GPS_names, GPS_values, coordinate_names, coordinate_values));
    ...
    IEnumerator GetGlobalMetadata(string url, string key, string pin_entryID, string[] GPS_names, string[] GPS_values, string[] coordinate_names, string[] coordinate_values) {

      for (int current=0; current<GPS_names.Length; current++) {
        var get_url = url + "/get?key=" + key + "&data=" + GPS_names[current];
        using (UnityWebRequest webRequest = UnityWebRequest.Get(get_url)) {
          ...
          if (webRequest.isNetworkError) {
              ...
              string val = webRequest.downloadHandler.text;
              GPS_values[current] = val;
          }
        }
      }
